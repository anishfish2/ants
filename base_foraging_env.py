...
class parallel_env(ParallelEnv):
    ...
    def __init__(self, render_mode=None):
        ...
        self.food = [food({'x':20, 'y':20}) for _ in range(num_food)]
    ...
    def reset(self, seed=None, options=None):
        ...
        self.food = [food({'x':20, 'y':20}) for _ in range(num_food)]
        self.food_location_log = {food: [] for food in self.food}
    ...
    def step(self, actions):
        ...
        rewards = {agent: 1 if np.linalg.norm(np.array((self.food[0].location['x'], self.food[0].location['y'])) - np.array((agent.location['x'], agent.location['y']))) < np.linalg.norm(np.array((self.food[0].location['x'], self.food[0].location['y'])) - np.array((agent_previous_location[agent]['x'], agent_previous_location[agent]['y']))) else 0 for agent in self.agents}
            ...
            for i, frame in enumerate(self.agent_location_log[self.agents[0]]):
                ...
                for food in self.food:
                    if i < len(self.food_location_log[food]):
                        ax.scatter(self.food_location_log[food][i]['x'], self.food_location_log[food][i]['y'], color='green')
